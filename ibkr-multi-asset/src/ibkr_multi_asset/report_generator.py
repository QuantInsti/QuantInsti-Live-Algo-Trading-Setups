...
def _extract_account_series(account_summary, tags):
    if account_summary.empty or "Tag" not in account_summary.columns or "Value" not in account_summary.columns:
        return pd.Series(dtype=float)
    acc = _ensure_datetime_column(account_summary)
    if "datetime" not in acc.columns:
        return pd.Series(dtype=float)
    acc = acc.dropna(subset=["datetime"])
    acc["Tag"] = acc["Tag"].astype(str)
    acc["Value"] = _to_numeric(acc["Value"])
    subset = acc[acc["Tag"].isin(tags)].dropna(subset=["Value"])
    if subset.empty:
        return pd.Series(dtype=float)
...
def _build_account_diagnostics(account_summary, account_updates, cash_balance, holdings_metrics):
    ...
    net_liq = _extract_account_series(account_summary, ["NetLiquidation", "NetLiquidationByCurrency"])
    if net_liq.empty:
    ...
    available_funds = _extract_account_series(account_summary, ["AvailableFunds", "AvailableFunds-C"])
    if available_funds.empty:
        available_funds = _extract_account_updates_series(account_updates, ["AvailableFunds", "AvailableFunds-C"])
    excess_liquidity = _extract_account_series(account_summary, ["ExcessLiquidity", "ExcessLiquidity-C"])
    if excess_liquidity.empty:
        excess_liquidity = _extract_account_updates_series(account_updates, ["ExcessLiquidity", "ExcessLiquidity-C"])
    buying_power = _extract_account_series(account_summary, ["BuyingPower"])
    if buying_power.empty:
        buying_power = _extract_account_updates_series(account_updates, ["BuyingPower"])
    gross_position = _extract_account_series(account_summary, ["GrossPositionValue"])
    if gross_position.empty:
...
def _build_operational_diagnostics(periods_traded, runtime_audit, ib_errors, app_time_spent):
    traded_symbols = []
    completion_ratio = np.nan
    missed_periods = pd.DataFrame()
    period_heatmap = pd.DataFrame()

    if not periods_traded.empty and {"Symbol", "trade_time", "trade_done"}.issubset(periods_traded.columns):
        ...
        periods["Symbol"] = periods["Symbol"].astype(str).str.upper()
        periods["trade_done"] = _to_numeric(periods["trade_done"]).fillna(0.0)
        traded_symbols = sorted(periods["Symbol"].unique().tolist())
        completion_ratio = float((periods["trade_done"] == 1).mean()) if len(periods) else np.nan
        missed_periods = periods[periods["trade_done"] != 1].sort_values(["trade_time", "Symbol"])
        pivot = periods.copy()
        pivot["trade_date"] = pivot["trade_time"].dt.strftime("%Y-%m-%d %H:%M")
        period_heatmap = pivot.pivot_table(index="Symbol", columns="trade_date", values="trade_done", aggfunc="max")
        period_heatmap = period_heatmap.sort_index(axis=0).sort_index(axis=1)

    runtime_counts = pd.Series(dtype=float)
    recent_runtime = pd.DataFrame()
    if not runtime_audit.empty and {"event", "datetime"}.issubset(runtime_audit.columns):
        audit = runtime_audit.copy()
        audit["datetime"] = pd.to_datetime(audit["datetime"], errors="coerce")
        audit = audit.dropna(subset=["datetime"])
        audit["event"] = audit["event"].astype(str)
        runtime_counts = audit["event"].value_counts().sort_values(ascending=False)
        recent_runtime = audit.sort_values("datetime").tail(20)

    error_counts = pd.Series(dtype=float)
    recent_errors = pd.DataFrame()
    if not ib_errors.empty and {"Code", "datetime"}.issubset(ib_errors.columns):
        errs = ib_errors.copy()
        errs["datetime"] = pd.to_datetime(errs["datetime"], errors="coerce")
        errs = errs.dropna(subset=["datetime"])
        errs["Code"] = errs["Code"].astype(str)
        error_counts = errs["Code"].value_counts().sort_values(ascending=False)
        recent_errors = errs.sort_values("datetime").tail(20)

    runtime_seconds = pd.Series(dtype=float)
    if not app_time_spent.empty and {"Symbol", "seconds"}.issubset(app_time_spent.columns):
        spent = app_time_spent.copy()
        spent["seconds"] = _to_numeric(spent["seconds"])
        spent["Symbol"] = spent["Symbol"].astype(str).str.upper()
        runtime_seconds = spent.groupby("Symbol")["seconds"].last().sort_values()
    ...
    return {
        ...
        "metrics": {
            ...
            "error_count": int(error_counts.sum()) if len(error_counts) else 0,
            "runtime_event_count": int(runtime_counts.sum()) if len(runtime_counts) else 0,
...
def _flatten_state(value, prefix=""):
    if isinstance(value, dict):
        ...
        for key, val in value.items():
            rows.extend(_flatten_state(val, f"{prefix}.{key}" if prefix else str(key)))
        return rows
    if isinstance(value, (list, tuple)):
        preview = ", ".join(str(x) for x in list(value)[:6])
        if len(value) > 6:
            preview += ", ..."
...
def _build_strategy_state_diagnostics(strategy_state, holdings, final_equity):
    rows = []
    target_rows = []
    latest = pd.DataFrame()
    if not strategy_state.empty and {"Symbol", "Scope", "StateKey", "StateValue"}.issubset(strategy_state.columns):
        latest = _ensure_datetime_column(strategy_state)
        if "datetime" in latest.columns:
            latest = latest.dropna(subset=["datetime"]).sort_values("datetime")
        latest = latest.drop_duplicates(subset=["Symbol", "Scope", "StateKey"], keep="last")
        latest["parsed"] = latest["StateValue"].apply(_parse_state_value)
        for row in latest.itertuples(index=False):
            for flat_key, flat_val in _flatten_state(getattr(row, "parsed"), f"{row.Scope}.{row.StateKey}"):
                rows.append(
                    {
            ...
            state_key = str(row.StateKey).lower()
            parsed = getattr(row, "parsed")
            if isinstance(parsed, dict):
                symbol = str(row.Symbol).upper()
                tgt_qty = parsed.get("target_quantity")
                exposure = parsed.get("exposure")
                share = parsed.get("share")
                signal = parsed.get("signal")
                if any(x is not None for x in [tgt_qty, exposure, share, signal]):
                    target_rows.append(
                        {
                            "Symbol": symbol,
                            "Scope": str(row.Scope),
                            "StateKey": str(row.StateKey),
                            "Signal": signal,
            ...
            elif any(x in state_key for x in ["target", "weight", "signal", "leverage", "exposure"]):
                target_rows.append(
                    {
                        "Symbol": str(row.Symbol).upper(),
                        "Scope": str(row.Scope),
                        "StateKey": str(row.StateKey),
                        "Signal": parsed if "signal" in state_key else None,
                        "TargetQuantity": parsed if "quantity" in state_key else None,
                        "TargetExposure": parsed if any(x in state_key for x in ["exposure", "leverage"]) else None,
                        "TargetShare": parsed if "weight" in state_key or "share" in state_key else None,
                    }
                )

    state_table = pd.DataFrame(rows).sort_values(["Symbol", "Key"]) if rows else pd.DataFrame(columns=["Symbol", "Key", "Value", "datetime"])
    ...
    if not target_df.empty:
        ...
        target_df = target_df.groupby("Symbol", as_index=False).agg(agg)
        if not holdings.empty:
            actual = holdings[["Symbol", "Position", "Weight", "MarketValue"]].copy()
            actual["ActualExposure"] = actual["MarketValue"].abs() / abs(final_equity) if final_equity else np.nan
            target_df = target_df.merge(actual[["Symbol", "Position", "Weight", "ActualExposure"]], on="Symbol", how="left")
            target_df["QuantityError"] = _to_numeric(target_df["TargetQuantity"]) - _to_numeric(target_df["Position"])
            target_df["ExposureError"] = _to_numeric(target_df["TargetExposure"]) - _to_numeric(target_df["ActualExposure"])

    return {"state_table": state_table, "target_df": target_df}
...
def generate_live_portfolio_report(app_instance, output_path="data/portfolio_report.pdf"):
    ...
    try:
        ...
        account_summary = _load_sheet(database_path, "account_summary")
        app_time_spent = _load_sheet(database_path, "app_time_spent")
        runtime_audit = _load_sheet(database_path, "runtime_audit")
        ib_errors = _load_sheet(database_path, "ib_errors")
        strategy_state = _load_sheet(database_path, "strategy_state")
        ...
        account_diag = _build_account_diagnostics(account_summary, account_updates, cash_balance, holdings_diag["metrics"])
        ops_diag = _build_operational_diagnostics(periods_traded, runtime_audit, ib_errors, app_time_spent)
        state_diag = _build_strategy_state_diagnostics(strategy_state, holdings_diag["holdings"], final_cap)
        ...
        traded_symbols = ops_diag["traded_symbols"]
        active_assets = len(traded_symbols)
        ...
        with PdfPages(output_path) as pdf:
            ...
            summary_lines = [
                ...
                f"Traded Symbols: {', '.join(traded_symbols) if traded_symbols else 'n/a'}",
            ...
            axes[0, 0].set_title("Runtime Seconds by Symbol")

            runtime_counts = ops_diag["runtime_counts"]
            if len(runtime_counts):
                runtime_counts.head(12).plot(kind="bar", ax=axes[0, 1], color="tab:green")
                axes[0, 1].tick_params(axis="x", rotation=30)
            else:
                axes[0, 1].text(0.5, 0.5, "No runtime audit data", ha="center", va="center")
            axes[0, 1].set_title("Runtime Event Counts")

            error_counts = ops_diag["error_counts"]
            if len(error_counts):
                error_counts.head(12).plot(kind="bar", ax=axes[1, 0], color="tab:red")
                axes[1, 0].tick_params(axis="x", rotation=30)
            else:
                axes[1, 0].text(0.5, 0.5, "No IB errors logged", ha="center", va="center")
            axes[1, 0].set_title("IB Error Codes")
            ...
            ops_lines = [
                ...
                f"Runtime Events Logged: {ops_diag['metrics']['runtime_event_count']}",
                f"IB Errors Logged: {ops_diag['metrics']['error_count']}",
            ]
            axes[1, 1].text(0.03, 0.97, "\n".join(ops_lines), va="top", family="monospace")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

            if not ops_diag["period_heatmap"].empty:
                heat = ops_diag["period_heatmap"]
                max_cols = min(30, heat.shape[1])
                heat = heat.iloc[:, -max_cols:]
                fig, ax = plt.subplots(figsize=(max(11, 0.4 * max_cols), 8.5))
                sns.heatmap(heat, cmap="RdYlGn", cbar=False, vmin=0, vmax=1, annot=False, ax=ax)
                ax.set_title("Recent Period Completion Heatmap")
                ax.set_xlabel("Trade Time")
                ax.set_ylabel("Symbol")
                pdf.savefig(fig, bbox_inches="tight")
                plt.close(fig)

            target_df = state_diag["target_df"]
            fig, axes = plt.subplots(2, 2, figsize=(11, 8.5))
            if not target_df.empty and "ExposureError" in target_df.columns:
                t = target_df.sort_values("ExposureError").tail(12)
                colors = np.where(_to_numeric(t["ExposureError"]).fillna(0.0) >= 0, "tab:green", "tab:red")
                axes[0, 0].barh(t["Symbol"], _to_numeric(t["ExposureError"]).fillna(0.0), color=colors)
            else:
                axes[0, 0].text(0.5, 0.5, "No target-vs-actual exposure data", ha="center", va="center")
            axes[0, 0].set_title("Exposure Tracking Error")

            if not target_df.empty and "QuantityError" in target_df.columns:
                t = target_df.sort_values("QuantityError").tail(12)
                colors = np.where(_to_numeric(t["QuantityError"]).fillna(0.0) >= 0, "tab:green", "tab:red")
                axes[0, 1].barh(t["Symbol"], _to_numeric(t["QuantityError"]).fillna(0.0), color=colors)
            else:
                axes[0, 1].text(0.5, 0.5, "No target-vs-actual quantity data", ha="center", va="center")
            axes[0, 1].set_title("Quantity Tracking Error")

            state_table = state_diag["state_table"]
            if not state_table.empty:
                top_keys = state_table["Key"].value_counts().head(10)
                top_keys.plot(kind="bar", ax=axes[1, 0], color="tab:blue")
                axes[1, 0].tick_params(axis="x", rotation=35)
            else:
                axes[1, 0].text(0.5, 0.5, "No strategy state persisted", ha="center", va="center")
            axes[1, 0].set_title("Most Persisted Strategy-State Keys")

            axes[1, 1].axis("off")
            state_lines = [
                "Strategy State",
                "",
                f"State Entries: {len(state_table)}",
                f"Symbols with Targets: {int(target_df['Symbol'].nunique()) if not target_df.empty else 0}",
            ]
            if not target_df.empty and "ExposureError" in target_df.columns:
                state_lines.append(f"Max |Exposure Error|: {_safe_pct(_to_numeric(target_df['ExposureError']).abs().max())}")
            if not target_df.empty and "QuantityError" in target_df.columns:
                state_lines.append(f"Max |Quantity Error|: {_safe_num(_to_numeric(target_df['QuantityError']).abs().max(), 2)}")
            axes[1, 1].text(0.03, 0.97, "\n".join(state_lines), va="top", family="monospace")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)
