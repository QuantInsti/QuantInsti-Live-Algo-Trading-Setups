...
class trading_app(EClient, EWrapper):
    def __init__(self, logging, account, account_currency, symbol, timezone, data_frequency, historical_data_address, base_df_address,
        ...
        self.account_summary_df = self._load_sheet(database, 'account_summary', index_col=0, parse_index_datetime=True)
        self.account_update_times_df = self._load_sheet(database, 'account_update_times', index_col=0, parse_index_datetime=True)
        self.contract_details_df = self._load_sheet(database, 'contract_details', index_col=None)
        self.ib_errors = self._load_sheet(database, 'ib_errors', index_col=0)
        self.runtime_audit = self._load_sheet(database, 'runtime_audit', index_col=0)
        self.strategy_state_df = self._load_sheet(database, 'strategy_state', index_col=0)

        self.app_time_spent_all = self._load_sheet(database, 'app_time_spent', index_col=0, parse_index_datetime=True)
        if 'Symbol' not in self.app_time_spent_all.columns:
            self.app_time_spent_all['Symbol'] = 'PORTFOLIO'
        self.app_time_spent_all['Symbol'] = self.app_time_spent_all['Symbol'].astype(str).str.upper()
        ...
        self.app_time_spent = self.app_time_spent_all[self.app_time_spent_all['Symbol'] == 'PORTFOLIO'].copy()
        ...
        if 'Symbol' not in self.periods_traded_all.columns:
            self.periods_traded_all['Symbol'] = 'PORTFOLIO'
        ...
        self.periods_traded_all['Symbol'] = self.periods_traded_all['Symbol'].astype(str).str.upper()
        self.periods_traded = self.periods_traded_all[self.periods_traded_all['Symbol'] == 'PORTFOLIO'].copy()
        ...
        self.temp_account_summary = pd.DataFrame(columns=WORKBOOK_SCHEMAS['account_summary'])
        self.temp_account_update_times = pd.DataFrame(columns=WORKBOOK_SCHEMAS['account_update_times'])
        self.temp_contract_details = pd.DataFrame(columns=WORKBOOK_SCHEMAS['contract_details'])
        self.temp_ib_errors = pd.DataFrame(columns=WORKBOOK_SCHEMAS['ib_errors'])
        self.temp_runtime_audit = pd.DataFrame(columns=WORKBOOK_SCHEMAS['runtime_audit'])
        self.temp_strategy_state = pd.DataFrame(columns=WORKBOOK_SCHEMAS['strategy_state'])
    ...
    def error(self, reqId, code_or_time, msg_or_code, *args, **kwargs):
        ...
        self.temp_ib_errors = pd.concat([self.temp_ib_errors, pd.DataFrame(dictionary, index=[0])], ignore_index=True)
    ...
    def openOrderEnd(self):
        if not (bool(getattr(self, 'silent_broker_sync', False)) or int(getattr(self, 'silent_broker_sync_depth', 0)) > 0):
    ...
    def updateAccountTime(self, timeStamp: str):
        print(f'[{self.ticker}] Account update time is: {timeStamp}')
        dictionary = {
    ...
    def accountDownloadEnd(self, accountName: str):
        print(f'[{self.ticker}] Account download was done for account: {accountName}')
    ...
    def contractDetails(self, reqId, contractDetails):
        ...
        for attr in ('symbol', 'secType', 'currency', 'exchange', 'primaryExchange', 'localSymbol', 'tradingClass', 'multiplier'):
