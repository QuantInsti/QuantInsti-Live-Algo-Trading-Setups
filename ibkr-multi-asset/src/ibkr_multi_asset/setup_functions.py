...
def _repair_account_update_times_frame(frame, current_period=None):
    local = _frame_with_datetime_column(frame)
    if local.empty:
        return pd.DataFrame(columns=WORKBOOK_SCHEMAS['account_update_times'])
...
def _flush_runtime_audit_buffer(app):
    ...
    app.temp_runtime_audit = pd.DataFrame(columns=WORKBOOK_SCHEMAS['runtime_audit'])
...
def _current_open_orders_snapshot(app):
    snapshot = getattr(app, 'current_open_orders_snapshot', pd.DataFrame())
...
def request_contract_details(app, verbose=True):
    ...
    def _hydrate_contract_from_row(row):
        ...
        for attr, column in (
            ...
            if column in row and pd.notna(row[column]) and str(row[column]) != '':
                setattr(contract, attr, row[column])
        if 'ConId' in row and pd.notna(row['ConId']):
            try:
                contract.conId = int(float(row['ConId']))
            except Exception:
                pass
        return contract
    ...
    if not existing.empty and 'Symbol' in existing.columns:
        ...
        if not same_symbol.empty:
            ...
            candidate_row = candidate_rows.iloc[-1]
            if _contract_row_is_resolved(candidate_row, asset_class):
                app.contract = _hydrate_contract_from_row(candidate_row)
    ...
    if not app.temp_contract_details.empty:
        ...
        if not same_symbol.empty:
            ...
            resolved_rows = same_symbol[same_symbol.apply(lambda row: _contract_row_is_resolved(row, asset_class), axis=1)]
            candidate_rows = resolved_rows if not resolved_rows.empty else same_symbol
            app.contract = _hydrate_contract_from_row(candidate_rows.iloc[-1])
...
def update_hist_data(app):
    ...
    request_days = max(days_passed_number if days_passed_number > 1 else (days_passed_number + 2), bootstrap_days)
...
def _normalize_order_quantity(app, quantity):
    ...
    decimals = max(0, len(format(step, 'f').rstrip('0').split('.')[-1]) if '.' in format(step, 'f').rstrip('0') else 0)
...
def update_capital(app):
    ...
    shared_account_summary = getattr(app, 'shared_account_summary_df', None)
    shared_account_update_times = getattr(app, 'shared_account_update_times_df', None)
    if isinstance(shared_acc_update, pd.DataFrame) and not shared_acc_update.empty:
        ...
        if isinstance(shared_account_summary, pd.DataFrame) and not shared_account_summary.empty:
            app.account_summary_df = shared_account_summary.copy(deep=True)
        if isinstance(shared_account_update_times, pd.DataFrame) and not shared_account_update_times.empty:
            app.account_update_times_df = shared_account_update_times.copy(deep=True)
        ...
        app.account_summary_event.wait(timeout=30)
        app.cancelAccountSummary(7001)
        time.sleep(1)
        ...
        _flush_temp_sheet(app, 'temp_account_summary', 'account_summary_df', 'account_summary', dedupe_subset=['datetime', 'ReqId', 'Account', 'Tag', 'Currency'])
        _flush_temp_sheet(app, 'temp_account_update_times', 'account_update_times_df', 'account_update_times', dedupe_subset=['datetime', 'Account', 'UpdateTime'])
        print('Account values successfully updated ......')
        app.logging.info('Account values successfully requested...')
    else:
        return

    if app.acc_update.empty or app.acc_update[(app.acc_update['key'] == 'TotalCashBalance') & (app.acc_update['Currency'] == 'BASE')].empty:
        append_runtime_audit(app, 'account_update_missing', 'No TotalCashBalance BASE update received from IB')
        app.logging.warning('No TotalCashBalance BASE update received from IB; keeping previous capital value.')
        return
...
def get_current_quantity(app):
    ...
    if quantity_mode in {'contracts', 'units', 'quantity'} and pd.notna(target_quantity):
        app.current_quantity = abs(float(target_quantity))
...
def _risk_management_position_sign(app):
    ...
    if np.isfinite(live_quantity) and live_quantity != 0:
        return float(np.sign(live_quantity))
...
def _wait_for_live_position(app, expected_sign=None, timeout_seconds=CRYPTO_FILL_WAIT_SECONDS):
    ...
    while time.time() < deadline:
        ...
        if not np.isclose(live_quantity, 0.0, atol=1e-8):
...
def _latest_position_for_symbol(app, refresh=True, verbose=True):
    if refresh and app.isConnected():
        request_positions(app, verbose=verbose)
    positions = getattr(app, 'pos_df', None)
    if not isinstance(positions, pd.DataFrame) or positions.empty:
        return 0.0
...
def send_orders_as_bracket(app, order_id, quantity, mkt_order, sl_order, tp_order, rm_quantity=None):
    ...
    if (mkt_order==True) and (sl_order==True) and (tp_order==True):
        ...
        if _is_crypto_asset(app):
            ...
            if np.isclose(confirmed_live_quantity, 0.0, atol=1e-8):
        ...
        try:
            ...
            send_take_profit_order(app, _next_bracket_order_id(app, sl_id), risk_qty)
...
def restore_carry_risk_management(app):
    ...
    if not np.isclose(refreshed_quantity, 0.0, atol=1e-8):
...
def _reconcile_direct_target_position(app, order_id):
    ...
    if app.risk_management_bool and not np.isclose(effective_signed_quantity, 0.0, atol=1e-8):
        if executable_delta_quantity <= 0:
            cancel_risk_management_previous_orders(app)
...
def send_orders(app):
    ...
    if not np.isfinite(float(app.previous_signal)):
        app.previous_signal = float(np.sign(app.previous_quantity)) if float(app.previous_quantity) != 0.0 else 0.0
...
def strategy(app):
    ...
    if app.isConnected() and not base_df.empty:
        ...
        if targets is not None:
            ...
            target = targets.get(symbol_key, {"signal": 0, "leverage": 0.0})
            app.signal = int(np.sign(float(target.get("signal", 0.0))))
            app.leverage = float(target.get("leverage", 0.0))
        else:
            if isinstance(results_signal, dict):
                app.signal = int(np.sign(float(results_signal.get('signal', 0.0))))
                app.leverage = float(results_signal.get('leverage', effective_vars.get('leverage', 1.0) or 1.0))
            else:
...
def collect_shared_account_snapshot(apps):
    ...
    shared_account_summary = lead_app.account_summary_df.copy(deep=True)
    shared_account_update_times = lead_app.account_update_times_df.copy(deep=True)
    for app in apps:
        ...
        app.shared_account_summary_df = shared_account_summary.copy(deep=True)
        app.shared_account_update_times_df = shared_account_update_times.copy(deep=True)
...
def save_portfolio_cycle_data(apps, send_email_summary=True):
    ...
    lead_app.account_summary_df = _merge_unique_frames([app.account_summary_df for app in apps], sort_index=True, dedupe_subset=['datetime', 'ReqId', 'Account', 'Tag', 'Currency'] if all(hasattr(app, 'account_summary_df') for app in apps) else None)
    lead_app.account_update_times_df = _merge_unique_frames([app.account_update_times_df for app in apps], sort_index=True, dedupe_subset=['datetime', 'Account', 'UpdateTime'] if all(hasattr(app, 'account_update_times_df') for app in apps) else None)
    ...
    lead_app.ib_errors = _merge_unique_frames([app.ib_errors for app in apps], sort_index=True)
    lead_app.runtime_audit = _merge_unique_frames([app.runtime_audit for app in apps], sort_index=True)
    ...
    lead_app.app_time_spent_all = _merge_unique_frames(
        ...
        dedupe_subset=['Symbol', 'datetime'] if 'datetime' in current_time_spent.columns else None,
    )
    ...
    lead_app.periods_traded_all = _merge_unique_frames(
        ...
        dedupe_subset=['Symbol', 'trade_time'] if 'trade_time' in current_period_row.columns else None,
    )
...
def save_week_open_and_close_datetimes(app, verbose=True):
    ...
    for dataframe in [app.open_orders, app.orders_status, app.exec_df, app.comm_df, \
                      app.pos_df, app.portfolio_snapshots_df, app.cash_balance, app.account_summary_df, app.account_update_times_df]:
...
def save_data(app, verbose=True):
    ...
    app.account_summary_df = _normalize_sheet_to_datetime_index(getattr(app, 'account_summary_df', pd.DataFrame()), invalid_before_year=2000)
    app.account_update_times_df = _repair_account_update_times_frame(getattr(app, 'account_update_times_df', pd.DataFrame()), current_period=getattr(app, 'current_period', None))
    _flush_runtime_audit_buffer(app)
    _flush_strategy_state_buffer(app)
    app.cash_balance.ffill(inplace=True)
    ...
    if not app.app_time_spent_all.empty:
        ...
        if 'Symbol' not in app.app_time_spent_all.columns:
            app.app_time_spent_all['Symbol'] = 'PORTFOLIO'
        if 'datetime' not in app.app_time_spent_all.columns:
            app.app_time_spent_all['datetime'] = pd.NaT
        app.app_time_spent_all['Symbol'] = app.app_time_spent_all['Symbol'].astype(str).str.upper()
        app.app_time_spent_all.sort_values(['datetime', 'Symbol'], inplace=True, ignore_index=True)
        app.app_time_spent_all.drop_duplicates(subset=['Symbol', 'datetime'], keep='last', inplace=True)
    ...
    if not app.periods_traded_all.empty:
        ...
        if 'Symbol' not in app.periods_traded_all.columns:
            app.periods_traded_all['Symbol'] = 'PORTFOLIO'
        if 'datetime' not in app.periods_traded_all.columns:
            app.periods_traded_all['datetime'] = pd.NaT
        app.periods_traded_all['Symbol'] = app.periods_traded_all['Symbol'].astype(str).str.upper()
        app.periods_traded_all['trade_time'] = pd.to_datetime(app.periods_traded_all['trade_time'], errors='coerce')
        app.periods_traded_all.sort_values(['trade_time', 'Symbol'], inplace=True, ignore_index=True)
        app.periods_traded_all.drop_duplicates(subset=['Symbol', 'trade_time'], keep='last', inplace=True)
    ...
    if not app.temp_ib_errors.empty:
        app.ib_errors = _frame_with_datetime_column(getattr(app, 'ib_errors', pd.DataFrame()))
        if not app.ib_errors.empty and 'datetime' in app.ib_errors.columns:
            app.ib_errors['datetime'] = pd.to_datetime(app.ib_errors['datetime'], errors='coerce')
            app.ib_errors = app.ib_errors.dropna(subset=['datetime'])
            app.ib_errors = app.ib_errors.reset_index(drop=True).set_index('datetime')
            app.ib_errors.index.name = ''
        error_df = app.temp_ib_errors.copy()
        error_df['datetime'] = pd.to_datetime(error_df['datetime'], errors='coerce')
        error_df = error_df.dropna(subset=['datetime'])
        error_df.set_index('datetime', inplace=True)
        error_df.index.name = ''
        app.ib_errors = pd.concat([app.ib_errors, error_df])
        app.ib_errors.drop_duplicates(inplace=True)
        app.ib_errors.sort_index(ascending=True, inplace=True)
        app.temp_ib_errors = pd.DataFrame(columns=app.ib_errors.columns)
    ...
    try:
        persisted_state_df = pd.read_excel(app.database_path, sheet_name='strategy_state')
        unnamed = [col for col in persisted_state_df.columns if str(col).startswith('Unnamed:')]
        if unnamed:
            persisted_state_df = persisted_state_df.drop(columns=unnamed)
        if not persisted_state_df.empty and 'datetime' in persisted_state_df.columns:
            persisted_state_df['datetime'] = pd.to_datetime(persisted_state_df['datetime'], errors='coerce')
            persisted_state_df = persisted_state_df.dropna(subset=['datetime'])
            persisted_state_df.set_index('datetime', inplace=True)
            persisted_state_df.index.name = ''
            app.strategy_state_df = pd.concat([_normalize_strategy_state_frame(getattr(app, 'strategy_state_df', pd.DataFrame())), persisted_state_df])
            app.strategy_state_df = _normalize_strategy_state_frame(app.strategy_state_df)
    except Exception:
        pass
    ...
    dictfiles = {'open_orders': app.open_orders,
                 ...
                 'app_time_spent': app.app_time_spent_all,
                 'periods_traded': app.periods_traded_all,
                 'account_updates': app.account_updates_df,
                 'account_summary': app.account_summary_df,
                 'account_update_times': app.account_update_times_df,
                 'contract_details': app.contract_details_df,
                 'ib_errors': app.ib_errors,
                 'runtime_audit': app.runtime_audit,
                 'strategy_state': app.strategy_state_df}
...
def run_strategy(app):
    ...
    app.app_time_spent.loc[:, 'Symbol'] = app.ticker
