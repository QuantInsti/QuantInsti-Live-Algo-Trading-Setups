...
> A Python framework for automated forex trading using the Interactive Brokers API.

## Overview

This framework provides a complete algorithmic trading solution for forex markets using the Interactive Brokers API. It includes risk management, real-time data processing, and modular strategy development capabilities.
...
- **Pluggable Strategy Logic:** The setup is designed for you to implement your own strategies in the `user_config/strategy.py` file, which serves as a template.
- **Flexible Analysis:** The framework allows for the integration of various technical indicators and analysis techniques into your custom strategy.

### Data & Risk Management
- **Data Utilities:** Includes scripts for downloading historical data and handling real-time data streams.
- **Risk Control Functions:** Provides functions for managing risk, such as setting stop-loss and take-profit orders and calculating position sizes. You can customize the implementation of these functions in your strategy file.
...
2.  **Configure Parameters**: Open `user_config/main.py` to set up your trading parameters. This is where you'll define your account details, the forex pair to trade, data frequency, and other core settings.
...
4.  **Run the Bot**: Once configured, you can start the trading bot by running the following command from the `ibkr-forex` directory:
    ```bash
