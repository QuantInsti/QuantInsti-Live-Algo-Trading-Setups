...
#### This is your "Guidelines" document to modify and run the `ibkr-stock` trading code
...
#### This document provides instructions for you if you want to modify the source code of the `ibkr-stock` trading application, rebuild the package, and run your modified setup.
...
The `ibkr-stock` package is a Python-based trading application designed for stock trading with Interactive Brokers. Once you make changes to source code files, this guide focuses on enabling you to:
...
You should navigate to the root directory of the `ibkr-stock` project. This is the directory that contains the `src/` folder. You can inspect the Python files located in `src/ibkr_stock/` to decide which files and functions you want to modify.
...
1.  **Identify the files to modify**:
    ...
    * For **core application logic** (how the engine runs, data handling, IB interactions): You should edit files within `src/ibkr_stock/`.
...
```bash
cd path_to/ibkr-stock
```
*(You should replace `path_to/ibkr-stock` with the actual path to your project.)*
...
3.  **Build the package**:
    From the root directory of the `ibkr-stock` project, you should run:
    ```bash
    python -m build
    ```
    This command reads the `pyproject.toml` and creates the package. You will see output in your terminal indicating the build process. Upon completion, a `dist/` directory will be created (or updated) in your project root, containing files like `ibkr_stock-1.0.0-py3-none-any.whl` (the version number might differ).
...
```bash
pip install dist/ibkr_stock-1.0.0-py3-none-any.whl --force-reinstall
```
*(You should adjust the filename `ibkr_stock-1.0.0-py3-none-any.whl` if your built package has a different version or name. You can check the contents of your `dist/` folder.)*

This step updates the installed `ibkr_stock` library in your Python environment with the changes you made in the `src/` directory.
...
The application should now run with your modifications. You should check the console output and any generated log files (e.g., in `data/log/`) to verify your changes are active and behaving as expected. The `main.py` script in the `user_config` folder is set up to import and use the `ibkr_stock` package you just rebuilt and reinstalled.
...
By following these steps, you can effectively customize `ibkr-stock` to your trading needs. You should remember to always proceed with caution, especially when dealing with financial applications.
